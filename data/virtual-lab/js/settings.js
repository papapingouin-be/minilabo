...
function buildProcessingFromMeasurements(channel) {
  ...
  const lines = [
    ...
    'const bits = Number.isFinite(ctx.bits) ? Math.max(1, Math.round(ctx.bits)) : 10;',
    'const fullScale = Number.isFinite(ctx.fullScale) ? ctx.fullScale : Math.pow(2, bits) - 1;',
    'const referenceVoltage = Number.isFinite(ctx.referenceVoltage) ? ctx.referenceVoltage : null;',
    '',
    'function clampCounts(value, maxCounts) {',
    '  const finite = Number.isFinite(value) ? value : 0;',
    '  if (!Number.isFinite(maxCounts) || maxCounts <= 0) {',
    '    return 0;',
    '  }',
    '  return Math.min(Math.max(0, finite), maxCounts);',
    '}',
    '',
    'function dacCountsToVoltage(counts, options = {}) {',
    '  const localBits = Number.isFinite(options.bits) ? Math.max(1, Math.round(options.bits)) : bits;',
    '  const localReference = Number.isFinite(options.reference)',
    '    ? options.reference',
    '    : (Number.isFinite(options.vref) ? options.vref : (referenceVoltage ?? 1));',
    '  const maxCounts = localBits === bits ? fullScale : Math.pow(2, localBits) - 1;',
    '  if (!Number.isFinite(maxCounts) || maxCounts <= 0) {',
    '    return 0;',
    '  }',
    '  const clamped = clampCounts(counts, maxCounts);',
    '  return (clamped / maxCounts) * localReference;',
    '}',
    '',
    'function dacCountsToAcVrms(counts, options = {}) {',
    '  const peak = dacCountsToVoltage(counts, options);',
    '  return peak / Math.sqrt(2);',
    '}',
    '',
    "// Exemple : pour un DAC 12 bits (4095 pas) pilotant un convertisseur 0-10 V :",
    "//   const vrms = dacCountsToAcVrms(2048, { reference: 10 });",
    "//   Étapes : 2048 / 4095 ≈ 0,5 -> Vcrête ≈ 5 V, Vrms = 5 / Math.sqrt(2) ≈ 3,53 V.",
    "//   Ajustez ensuite 'scale' et 'offset' pour tenir compte de votre capteur ou transformateur.",
    '',
    'switch (measurement) {',
  ];
